...
@PasswordHasherQualifier
@RequestScoped
public class PasswordHasher implements HashHandler {
  ...
  private static int iterations(int cost) {
    if ((cost < 0) || (cost > 30))
      throw new IllegalArgumentException("Cost is out of range: " + cost);
    return 1 << cost;
  }
  ...
  public String hash(String value) {
    ...
    byte[] dk = pbkdf2(value.toCharArray(), salt, 1 << cost);
  ...
  @Override
  public boolean checkEquality(String value, String token) {
    Matcher m = layout.matcher(token);
    if (!m.matches()) throw new IllegalArgumentException("Invalid token format");
    int iterations = iterations(Integer.parseInt(m.group(1)));
    byte[] hash = Base64.getUrlDecoder().decode(m.group(2));
    byte[] salt = Arrays.copyOfRange(hash, 0, SIZE / 8);
    byte[] check = pbkdf2(value.toCharArray(), salt, iterations);
    int zero = 0;
