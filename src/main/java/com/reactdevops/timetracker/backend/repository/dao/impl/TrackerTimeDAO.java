...
public class TrackerTimeDAO implements TimeTrackerCRUDDAO {
  ...
  @Inject @UserDAOQualifier private UserDAO userDAO;

  private static final String CREATE_TRACKED_TIME_STATEMENT =
      "INSERT INTO tracked_times(description, start_time, end_time, user_id) VALUES (?, ?, ?, ?);";
  private static final String FIND_ALL_TRACKED_TIMES =
      "SELECT * FROM tracked_times where user_id = ?;";
  ...
  public void create(TrackedTimeEntity object) throws SQLException {
    try (Connection connection = dataSourceProvider.getDataSource().getConnection()) {
      PreparedStatement preparedStatement =
          connection.prepareStatement(CREATE_TRACKED_TIME_STATEMENT);
    ...
    try (Connection connection = dataSourceProvider.getDataSource().getConnection();
      ...
      while (resultSet.next()) {
        ...
        UserEntity userEntity =
            userDAO.read(userId).orElse(new UserEntity(1L, "Vania", "password"));
  ...
  private void prepareStatementFromObject(
      PreparedStatement preparedStatement, TrackedTimeEntity object) throws SQLException {
    int index = 0;
  ...
  public Table readReport() throws SQLException {
    ...
    try (Connection connection = dataSourceProvider.getDataSource().getConnection();
        Statement statement = connection.createStatement()) {
      ResultSet resultSet = statement.executeQuery(REPORT_QUERY);
      table = Table.read().db(resultSet);
    }
    return table;
  }
}
